...
func (r *Runner) buildUkrainianSubstitutions() error {
	...
	files := []struct {
		...
		{"replace.txt", "Barbarism.yml", generate.SubstitutionOptions{
			...
			Provenance: "Generated by stet from LanguageTool (LGPL-2.1). Do not edit.",
			// Vale fills the replacement first and the match second, so a
			// message naming the match first says the opposite of what it means.
			Message: "Калька або барбаризм. Українською: '%s' замість '%s'.",
